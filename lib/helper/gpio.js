...
class Gpio {
	 ...
	 */
	getEdge(channel) {
		return Gpio.readFile(Gpio.getChannelPath(channel, 'edge'));
	}
	...
	setEdge(channel, edge) {
		...
		}).then(() => Gpio.writeFile(Gpio.getChannelPath(channel, 'edge'), edge));
	}
	...
	getDirection(channel) {
		return Gpio.readFile(Gpio.getChannelPath(channel, 'direction'));
	}
	...
	setDirection(channel, direction) {
		return new Promise((resolve, reject) => {
			if (Gpio.direction.hasOwnProperty(direction) === false) {
				reject(new UnknownDirectionError(direction));
			} else {
				resolve();
			}
		}).then(() => Gpio.writeFile(Gpio.getChannelPath(channel, 'direction'), direction));
	}
	 ...
	 */
	getValue(channel) {
		return Gpio.readFile(Gpio.getChannelPath(channel, 'value'))
	...
	setValue(channel, value) {
		return Gpio.writeFile(Gpio.getChannelPath(channel, 'value'), value ? Gpio.signal.high : Gpio.signal.low);
	}
	 ...
	 * @returns {string}
	 */
	static getChannelPath(channel, path) {
		return `gpio${Gpio.pins[channel]}/${path}`;
	}
	...
	static writeFile(path, data) {
		return promisify(fs.writeFile)(`${Gpio.path}/${path}`, data, {
			encoding: 'utf8',
			flag: 'w+'
		}).catch((error) => console.error(error));
	}
 ...
 */
Gpio.pins = new Proxy({}, {
	get: (_, channel) => Gpio.mapping[String(channel)] || (() => {
