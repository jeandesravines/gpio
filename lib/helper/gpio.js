...
class Gpio {
	...
	constructor(path) {
		...
		/**
		 * @type {string} path to the gpio
		 * @readonly
		 * @private
		 */
		this._path = path || (fs.existsSync(Gpio.paths.v1) ? Gpio.paths.v1 : Gpio.paths.v2);
	}
	...
	readFile(path) {
		return new Promise((resolve, reject) => {
			fs.readFile(this._path + '/' + path, 'utf8', (error, data) => {
				error ? reject(error) : resolve(data.toString().trim());
			});
		});
	}
	...
	writeFile(path, data) {
		return new Promise((resolve, reject) => {
			fs.writeFile(this._path + '/' + path, data, 'utf8', (error) => {
