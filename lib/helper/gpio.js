...
class Gpio {
	...
	/**
	 * Get the CPU revision indicate in the file at path
	 * @param {string} path the path to search the CPU revision
	 * @returns {Number} the CPU revision
	 */
	static getRevision(path = '/proc/cpuinfo') {
		return fs.existsSync(path) ? parseInt((fs.readFileSync(path).toString()
			.match(/Revision\s*:\s*([0-9a-f]+)/) || []).pop(), 16) || 0 : 15;
	}
