...
class Gpio {
	...
	setDirection(channel, direction) {
		...
		}).then(() => Gpio.writeFile(this.getChannelPath(channel, 'direction'), direction));
	}
	...
	setValue(channel, value) {
		return Gpio.writeFile(this.getChannelPath(channel, 'value'), value ? Gpio.signal.high : Gpio.signal.low);
	}
	 ...
	 */
	static exists(path) {
		return Catcher.resolve(() => fs.lstatSync(path)) ? true : false;
	}
