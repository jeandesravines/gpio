...
const promisify = require('../module/promisify');
const fs = require('fs');
...
class Gpio {
	 ...
	 * Instantiate a Gpio
	 * @constructor
	 * @param {string} [path] path to the gpio directory
	 */
	constructor(path) {
		/**
		 ...
		 * @private
		 */
		this._path = path || (Gpio.exists(Gpio.paths.v1) ? Gpio.paths.v1 : Gpio.paths.v2);


		///////////////////////////////////////////////////////////
		///////////////////////////////////////////////////////////

		if (false === Gpio.exists(this._path)) {
			throw new NotFoundError(this._path);
		}
	}
	...
	setValue(channel, value, pwm = false) {
		...
		return this.writeFile(this.pins[channel] + '/value', value ? Gpio.signal.high : Gpio.signal.low);
	}
	 ...
	 */
	readFile(path) {
		return promisify(fs.readFile)(this._path + '/' + path, 'utf8');
	}
	 ...
	 */
	writeFile(path, data) {
		return promisify(fs.writeFile)(this._path + '/' + path, data, 'utf8');
	}
	 ...
	 */
	static get revision() {
		return singletons.revision || (singletons.revision = Gpio.exists('/proc/cpuinfo') ? parseInt(fs.readFileSync('/proc/cpuinfo')
