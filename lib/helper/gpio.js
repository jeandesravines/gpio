...
class Gpio {
	...
	close(channel) {
		...
		return this.setValue(channel, Gpio.signal.low)
		...
		return new Promise((resolve, reject) => {
			...
			this._readTimeouts[channel] = setTimeout(() => {
				...
				values.sort((a, b) => {
					...
					signals[value.signal ? 'high' : 'low'] += value.time - values[index ? index - 1 : Gpio.frame].time
				});
 ...
 */
Gpio.pins = new Proxy({}, {
	get: (proxy, channel) => Gpio.mapping[String(channel)] || (() => {
